...
/**
 * Repository layer for COVAN (advisor) related data access.
 */
export const covanRepository = {
  async myClasses(ma_co_van) {
    return query(
      `SELECT l.*, k.ten_khoa
       FROM LOP l
  ...
  async classStudents(ma_co_van, ma_lop) {
  ...
  async myReplacementRequests(ma_co_van) {
    return query(
